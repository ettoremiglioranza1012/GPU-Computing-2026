/* spmv_gpu_tpr.cu — GPU SpMV: Thread-Per-Row (COO format)
 *
 * Algorithm: one GPU thread per matrix row.
 *   thread row: binary search into sorted COO for first NNZ in row,
 *               then sequential scan to accumulate: y[row] = sum(Avals[i] * x[Acols[i]])
 * No atomicAdd needed — each thread writes exclusively to its own y[row].
 * Requires row-sorted COO (guaranteed by mtx_io.h).
 *
 ...
 *
 * Usage: ./bin/GPU/spmv_gpu_tpr.exec path/to/matrix.mtx
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
                          ...
                          float *y, int rows, int nnz)
{
    ...
    /* Binary search: find first NNZ index belonging to this row.
     * COO is row-sorted, so a standard leftmost-occurrence search works. */
    int lo = 0, hi = nnz - 1, start = nnz;
    while (lo <= hi) {
        int mid = lo + (hi - lo) / 2;
        if      (Arows[mid] < row) lo = mid + 1;
        else if (Arows[mid] > row) hi = mid - 1;
        else { start = mid; hi = mid - 1; }   /* keep searching left */
    }
